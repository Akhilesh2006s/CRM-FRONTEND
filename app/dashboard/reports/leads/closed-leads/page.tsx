...
export default function ReportsClosedLeadsPage() {
  ...
  const loadLeads = async () => {
    setLoading(true)
    try {
      const data = await apiRequest<Lead[]>('/leads?status=Closed')
      setAllLeads(data || [])
      
      // Extract unique zones
      const uniqueZones = Array.from(new Set(data.map(l => l.zone).filter(Boolean))) as string[]
  ...
  return (
    <div className="space-y-6 w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl md:text-3xl font-semibold text-neutral-900 whitespace-nowrap">Closed Leads List</h1>
