...
export default function ApprovedPaymentsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      toast.error(err?.message || 'Failed to load approved payments')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 lg:px-8 space-y-6">
      <Card className="p-6">
        <h1 className="text-2xl font-semibold">Approved Payments</h1>
            ...
            <TableBody>
              ...
              {rows.map((r, idx) => (
                <TableRow key={r._id}>
                  ...
                  <TableCell className="whitespace-nowrap">
                    {r.approvedAt
                      ? new Date(r.approvedAt).toLocaleString('en-GB', {
                          year: 'numeric',
                          month: '2-digit',
                          day: '2-digit',
                          hour: '2-digit',
                  ...
                  <TableCell className="whitespace-nowrap">{r.approvedBy?.name || '-'}</TableCell>
                  <TableCell className="whitespace-nowrap">{r.refNo || r.referenceNumber || '-'}</TableCell>
                  <TableCell className="whitespace-nowrap">{r.txnNo || '-'}</TableCell>
                  <TableCell className="truncate max-w-[150px]">{r.description || '-'}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
